...
注意：脚本会拒绝在有未提交本地改动的工作区继续执行；`data/`、`.env*` 不会被提交或覆盖。Git 拉取阶段只会更新版本差异里的文件；Docker 构建阶段会复用缓存，依赖安装层只受 `package.deps.json` 和 `bun.lock` 影响，单纯修改版本号或业务代码不会重新安装依赖。构建镜像会从官方 Bun 镜像复制 Bun 二进制文件，避免在 NAS 上通过 npm 全局安装 Bun。
